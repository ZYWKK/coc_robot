...
### 🤖 自动化操作
- ✅  可**后台运行**，不抢鼠标，运行后可以用电脑干别的事情，但不要把模拟器窗口最小化
- ✅ **主世界打鱼** - 自动搜索、进攻、收资源
- ✅ **速刷模式** ⚡ - 下兵后等待4-7秒直接放弃战斗，快速刷资源（约14秒/场）
- ✅ **夜世界打鱼** - 支持夜世界自动对战
...
- ✅ **命令行支持** - 支持脚本化调用
- ✅ **状态监控** - 自动检测程序状态并重启
- ✅ **企业微信通知** - 实时推送机器人状态到企业微信群（启动/停止/异常/定时上报）
- ✅ **公告横幅** 🆕 - 主界面顶部自动显示开发者公告（新版本提醒、已知问题等），点击可查看历史公告；网络不通时自动隐藏
- ✅ **操作模拟** - 基于贝塞尔曲线的鼠标轨迹模拟
- ✅ **漏下兵种处理** - 下完所有兵种后，依次检测前三个格子，若格子彩色（兵未下完），点击该格子选中兵种，对有效目标循环下兵，直到格子变灰为止。活动临时兵种可以正常下兵
...
- [📱 企业微信通知配置](docs/用户指南/企业微信通知.md) 🆕 - 远程监控机器人状态
...
**普通兵种**
> 野蛮人、弓箭手、巨人、哥布林、气球兵、法师、炸弹人、飞龙、皮卡超人、矿工、飞龙宝宝、野猪骑士、瓦基丽武神、女巫、天使、天狗、雷龙、大雪怪、龙骑士、根蔓骑士、蓝胖、石头人、苍蝇、戈仑冰人

**超级兵种**
> 超级野蛮人、超级飞龙、超级哥布林

**活动/赛季兵种**
> 扫帚女巫、蛮羊骑士、寒冰亡灵、收债哥布林、烟花炮手
...
**自动释放法术**（在所有兵种下完后循环释放）
- 镜像法术 - 在最后下兵位置释放
- 图腾法术、地震法术、蝙蝠法术、闪电法术、骷髅法术 、生日大爆炸法术（活动）- 在预设坐标随机释放
...
| 最小资源 | 数字 | `700000` | 进攻时要求敌方资源总量的最小值 |
| 资源建筑边缘比例 | 小数 | `0.5` | 资源建筑靠近地图边缘的比例（0-1，0表示不限制） |
| **是否快速刷资源** | 开关 | `关闭` |  速刷模式：下兵后等待4-7秒直接放弃战斗回营 |
| 是否刷天鹰火炮 | 开关 | `关闭` | 自动搜索天鹰火炮并使用雷电法术攻击（刷成就） |
| 是否开启刷墙 | 开关 | `关闭` | 是否使用多余资源升级城墙 |
| 刷墙起始金币 | 数字 | `100000` | 金币超过此值时触发刷墙 |
| 刷墙起始圣水 | 数字 | `100000` | 圣水超过此值时触发刷墙（低本建议设大值） |
| 是否刷主世界 | 开关 | `开启` | 是否启用主世界打鱼模式 |
| 是否刷夜世界 | 开关 | `关闭` | 是否启用夜世界打鱼模式 |
| 欲升级的英雄或建筑 | 多选 | 无 | 选择想要自动升级的英雄或建筑 |
| 是否升级建议升级的建筑 | 开关 | `开启` | 是否自动升级系统建议的建筑 |
| 建筑升级检查间隔 | 数字 | `0` | 检查建筑升级的时间间隔（小时，0表示每次都检查） |
...
| 是否采集进攻界面图像 | 开关 | `关闭` | 每次搜索村庄截图保存，用于训练YOLO模型（高级） |
...
```
最小资源：700000-1000000
边缘比例：0.5-0.6（只打外围采集器）
兵种配置：高效组合（超级哥布林+弓箭手+法师等）
法术配置：镜像+闪电+骷髅
```
...
```yaml
是否快速刷资源：✅ 开启
最小资源：100000-200000（降低要求，加快匹配）
边缘比例：0.0-0.3（放宽限制）

# 推荐配置（最快）
兵种配置：仅瓦基丽武神（下兵间隔50ms，速度最快）
法术配置：无（节省时间）

# 也可以使用的配置（稍慢但灵活）
兵种配置：瓦基丽武神 + 野蛮人/弓箭手等快速兵种
法术配置：可带地震法术等开城墙
```
...
**详细说明**：[📅 自动启动功能说明](docs/用户指南/自动启动功能.md)

### 方式二：命令行启动（高级用户）

如果你熟悉命令行，也可以使用传统方式手动编写启动脚本。

**详细说明**：[💻 命令行使用指南](docs/用户指南/命令行使用.md)
...
> 💡 **注意**：更新前建议备份你的配置数据库文件（`数据库/任务系统.db`）
...
<details>
<summary><b>Q: 速刷模式如何使用？</b></summary>

**速刷模式**是快速刷资源的功能，下兵后等待4-7秒直接放弃战斗，一场战斗约14秒完成。

**使用方法：**
1. 在配置面板勾选 **"是否快速刷资源（速刷模式）"**
2. 配置快速下兵的兵种
   - **最推荐**：仅瓦基丽武神（最快）
   - **可选**：超级野蛮人、野蛮人、弓箭手
3. 法术配置（可选）
   - **追求极速**：不带任何法术
   - **灵活配置**：可带镜像/闪电等（会正常释放，但会增加时间）
4. 降低"最小资源"要求，加快匹配速度
...
2. 确认Windows缩放为 `100%`
3. 关闭所有滤镜插件（MSI Afterburner、NVIDIA滤镜等）
4. 使用游戏默认皮肤，不要使用特殊背景
5. 检查兵种模板文件是否存在（`img/` 目录）
</details>
...
<details>
<summary><b>Q: 如何训练自己的YOLO模型？</b></summary>

项目自带的YOLO模型只能检测4种资源建筑（金矿、金库、圣水采集器、圣水瓶）。

如需检测其他目标（如天鹰火炮、大本营、兵种等），需要训练自己的模型：

1. 阅读 [YOLO检测器完整指南](docs/开发指南/06-YOLO检测器完整指南.md)
2. 开启"是否采集进攻界面图像"配置，自动收集训练数据
3. 使用标注工具标注图像
4. 训练YOLO模型
5. 替换项目中的模型文件

**详细教程**：[docs/开发指南/06-YOLO检测器完整指南.md](docs/开发指南/06-YOLO检测器完整指南.md)
</details>
